...
then the image $\sigma\im(\pp)$ is also prime for any $\sigma \in G$
(since $\sigma$ is an automorphism!).
Moreover, since $p \in \pp$ and $\sigma$ fixes $\QQ$,
we know that $p \in \sigma\im(\pp)$ as well.
...
\begin{proof}
	...
	\begin{align*}
		...
		% \pmod{(\sigma\inv)\im(\pp_2)} \text{ for $\sigma \in G$.}
	\end{align*}
