...
Although the above theorem is very handy to calculate the decomposition of the unramified prime $p$, it is hard to see why it works right off the bat.

To motivate this, we consider the special case when $E = \QQ(\alpha)$. Now let's go back a few chapters and think about the Factoring Algorithm.
...
To do this, suppose $S = \{ \alpha_1,\alpha_2,\cdots, \alpha_n\}$ are the roots of $f$ (distinct roots since $f$ is irreducible over $\QQ$). We let $\Frob_{\kP}$ act on $S$. This splits $S$ into orbits $S_1,S_2,\cdots,S_k$. Construct polynomials $f_i$ with coefficients in $E$ having roots exactly the elements of $S_i$. This forms a factorization of $f$ over $E$. \[
f = f_1f_2\cdots f_k
\] 
We claim that this in fact induces a factorization of $f \ (\mathrm{mod} \ p)$. 
...
We saw above that given the factorization pattern of $f \ (\mathrm{mod} \ p)$, we can determine the factorization pattern of an unramified prime $p$ in $\OO_E$.\\ 
