...
\url{https://blog.evanchen.cc/conjugate/}}.
...
\begin{problem}
	...
	\end{hint}
	\begin{sol}
		Proceed by contradiction and place the lamps at roots of unity
		(so that regular polygons have sum zero).
		Consider the sum of all moves.
		If we found a successful sequence of operations,
		then we have found a sequence $a_1$, $a_2$, \dots, $a_n$ of \emph{even} integers satisfying
		\[ 1 = \sum_i a_i e^{\frac{2\pi i}{n}}. \]
		In other words,
		\[ \half = \sum_i \frac{a_i}{2} e^{\frac{2\pi i}{n}}. \]
		This is a contradiction because the RHS is an algebraic integer
		(the sum of algebraic integers is also an algebraic integer),
		but the LHS $\half$ is not an algebraic integer.
	\end{sol}
\end{problem}
