...
Then by applying any embedding $\sigma$ to both sides gives
\begin{align*}
...
Specifically, in example (b) where we consider embeddings $K = \QQ(\sqrt 2)$ to $\CC$.
The image of these embeddings lands entirely in $K$: that is,
we could just have well as looked at $K \to K$ rather than looking at $K \to \CC$.
However, this is not true in (c): indeed $\QQ(\cbrt 2) \subset \RR$, but the non-identity embeddings
have complex outputs!
...
\end{definition}
Then, we can repeat the previous proof verbatim to obtain:
...
To tell you how the bijection goes, I have to define a fixed group.
\begin{definition}
