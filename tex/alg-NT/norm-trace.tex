...
This is adapted from by blog, \emph{Power Overwhelming}\footnote{URL: \url{https://usamo.wordpress.com/2014/10/19/why-do-roots-come-in-conjugate-pairs/}}.
...
You'll notice that the ``weird'' roots, like $1 \pm 2i$ and $2 \pm \sqrt 2$, are coming up in pairs. In fact, I think precalculus explicitly tells you that the imaginary roots come in conjugate pairs. More generally, it seems like all the roots of the form $a + b \sqrt c$ come in ``conjugate pairs''. And you can see why.
...
Now we know that if $2 + \sqrt 2$ is a root, then $A(x)$ is divisible by $P(x)$. And that's how we know that if $2 + \sqrt 2$ is a root of $A$, so must $2 - \sqrt 2$.
...
\begin{definition}
	...
	If so, consider the minimal polynomial of $\alpha$, which has coefficients in $\QQ$ but is monic.
...
\end{example}
\begin{proposition}[Rational Algebrac Integers are Rational Integers]
...
\begin{definition}
	...
	\[ \Tr_{K/\QQ}(\alpha) \defeq k \cdot \left( \prod \text{Galois conj of $\alpha$} \right). \]
\end{definition}
...
\begin{proposition}[Norms and Traces are Rational Integers]
	If $\alpha$ is an algebraic integer, its norm and trace are traces and integers.
\end{proposition}
\begin{ques}
	Prove it. (Vieta.)
\end{ques}
...
why is it the norm multiplicatve?
...
\begin{theorem}[Morally Correct Definition of Norm and Trace]
	...
	\end{itemize} 
\end{theorem}
Since the trace an determinant don't depend on the choice of basis,
...
\begin{subproof}[Proof if $k=1$]
	...
	\[
		...
		\right).
	\]
	Thus \[ \det M = (-1)^n c_0 \] and we're down by Vieta's formulas.
\end{subproof}
\begin{subproof}[Proof if $k > 1$]
	We have nested vector spaces
	\[ \QQ \subseteq \QQ(\alpha) \subseteq K. \]
	Let $e_1$, \dots, $e_k$ be an $\QQ(\alpha)$-basis for $K$
	...
	we just end up with one copy our old matrix for each $e_k$.
...
\begin{definition}
	Given a number field $K$, we define $\OO_K \defeq K \cap \ol\ZZ$ to be the \vocab{ring of integers} of $K$;
...
\begin{exercise}
	...
	\begin{itemize}
		...
		prove that $2a, a^2-db^2 \in \ZZ$ if and only if $a,b \in \ZZ$.
...
What we're going to show is that $\OO_K$ behaves a lot like the integers do in $\QQ$.
In particular, this is going to show that $\ol\ZZ$ behaves in $\ol\QQ$
in much the same way that $\ZZ$ behaves in $\QQ$.

First, in what way are integers ``integers''?
Because of this:
\begin{theorem}
...
\begin{exercise}
	Prove this yourself.
	(Start by using the fact that $\alpha$ has a minimal polynomial with integer coefficients.)
\end{exercise}

Now we are going to try and show $\OO_K$ is a ring;
we'll check it is closed under addition and multiplication. To do so, the easiest route is the following lemma.
\begin{definition}
...
In fact, something ever better is true.
...
\begin{theorem}[$\OO_K$ is a free $\ZZ$-module]
	Let $K$ be a number field of degree $n$.
	Then $\OO_K$ is a free $\ZZ$-module of rank $n$,
	\emph{id est} $\OO_K \cong \ZZ^{\oplus n}$ as an abelian group.
	In other words, $\OO_K$ is has a $\ZZ$-basis of $n$ elements as
	\[ \OO_K = \left\{ c_1\alpha_1 + \dots + c_{n-1}\alpha_{n-1} + c_n \mid c_i \in \ZZ \right\} \]
...
that we can write $\OO_K = \ZZ[\theta]$, in which case $\{1, \theta, \dots, \theta^{n-1}\}$
with serve both as a basis of $K$ and as the $\ZZ$-basis for $\OO_K$ (here $n = [K:\QQ]$).
This it is true for the most common examples we use:
\begin{itemize}
...
As we'll later see, monogenic extentsions have a really nice factoring algorithm,
\Cref{thm:factor_alg}.
...
\begin{problem}
	[USA TST 2012] Decide whether there exists $a,b,c > 2010$ satisfying
	\[ a^3+2b^3+4c^3=6abc+1. \]
\end{problem}

\begin{problem}
	[Black MOP 2010] There are $n > 2$ lamps arranged in a circle; initially one is on and the others are off.
...
\begin{problem}
	[Kronecker's Theorem] \yod Let $x$ be an algebraic integer.
