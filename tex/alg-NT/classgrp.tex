...
which measures how far something is from being a PID;
the bigger the class group, the farther it is from being a PID.
In particular, the PID's are the guys with trivial class group.
...
To make the next definition, we invoke the following theorem.
...
\begin{example}[Discriminant of $\QQ(i)$]
	...
	for number fields with wander into the complex plane (the lattice picture
	is a less perfect analogy).
...
We distinguish between the \vocab{real embeddings} -- those that map all of $K$ into $\RR$ --
and the \vocab{complex embeddings} -- those which map some part of $K$ outside $\RR$.
...
\begin{example}[Signatures for $K = \QQ(\cbrt{2})$]
...
\end{example}
\begin{ques}
	Verify that the signatures of the above two.
\end{ques}
...
Now, we can define the \vocab{canonical embedding} of $K$ by only setting
\begin{align*}
...
\]
by breaking each complex number into its real an imaginary part, as
\begin{align*}
...
\begin{example}[Example of Canonical Embedding]
	...
	noting that we've already arranged so $\sigma_1 = \id$ is real
	while $\sigma_2$ and $\sigma_3$ are a conjugate pair.
	So the embeddings $K \overset\iota\injto \RR \times \CC \taking\sim \RR^3$ is given by
	\[
...
\begin{proof}[Sketch of Proof]
	...
	To make this all precise I would need to do a lot with lattices and geometry
...
\begin{proof}[Sketch of Proof]
	...
	Since $S$ is symmetric, $-q \in S$ too. Then $\half (p-q)$ is in $S$ (convexity) and is a nonzero lattice point.
...
\begin{proof}
	...
	By \Cref{cor:class_rep_minkowski}, we can find a integral ideal $\bb \sim J$ with $\Norm(\bb) \le M_K$.
...
\begin{theorem}[Gaussian Integers have Unique Factorization]
...
Here's another example something that still turns out to be unique factorization,
...
\begin{proposition}
	The number field $K = \QQ(\sqrt{-17})$ has class group $\ZZ/4\ZZ$.
\end{proposition}
...
\begin{proof}
Since $D_K = -68$, we compute\footnote{
	Funny anecdote: when we did this at Harvard during Math 129,
	...
	about the difficulty of evaluating $\frac{4}{\pi} \sqrt{17}$. Excerpt: \\
		...
		``Even Gaitsgory [Math 55 professor] doesn't know that, how are we supposed to?'' -- Student 2 \\
...
From this we deduce $\Cl_K \cong \ZZ/4\ZZ$.
\end{proof}
...
\begin{problem}
	[China TST 1998] Let $n$ be a positive integer. A polygon in the plane (not necessarily convex) has area greater than $n$. Prove that one can translate it so that it contains at least $n+1$ lattice points.
	\begin{hint}
