...
\todo{add in more to this chapter}
...
\begin{definition}
	...
	then we say $A$ is a \vocab{subset} of $B$, and denote this by $A \subseteq B$.
...
\begin{definition}
	A function $f$ is:
	\begin{itemize}
		\ii \vocab{injective} if whenever $a_1 \neq a_2$, we have $f(a_1) = a_2$.
		Often, we will write $f : A \injto B$ to emphasize this.
		\ii \vocab{surjective} if whenever $b \in B$, there exists some $a \in A$
		such that $f(a) = b$.
		Often, we will write $f : A \surjto B$ to emphasize this.
		\ii \vocab{bijection} if both, i.e.\ $f$ is a ``one-to-one correspondence'':
...
This is called an \vocab{inverse function}; we denote it $f\inv : B \to A$.
...
\begin{definition}
	...
	The \vocab{image} $f``(S)$ is the set of all things of the form $f(s)$.
\end{definition}
\begin{example}
	[Examples of images]
...
An \vocab{equivalence relation} $\sim$ on $X$ is a binary relation
