...
The point of it is to just define what a vector space and a basis is.
These are very intuitive concepts that you likely already know.

\section{The Definition of a Ring and Field}
\prototype{$\ZZ$, $\RR$, or $\CC$ are all rings and the latter two are fields.}
...
\begin{quote}
	A ring is a structure with a \emph{commutative}
	addition, subtraction and multiplication, like $\ZZ$. 
	It also has a additive identity $0$ and multiplication identity $1$.
...
\prototype{Polynomials of degree at most $n$ form a vector space.}
...
\begin{example}
	...
	\begin{enumerate}[(a)]
		...
		forms a vector space, because you can add any two triples
...
\begin{definition}
	Let $R$ be a ring.
	A $R$-\vocab{module} is an additive abelian group $M = (M,+)$
	...
	\begin{enumerate}[(i)]
		...
		\ii $1_R \cdot m = m$.
		\ii $0_R \cdot m = 0_M$.\footnote{This condition is actually
...
\begin{example}
	...
	we see that \emph{any} ring $R$ can be thought of
...
\begin{definition}
	...
	of $M_1$ and $M_2$
	if every element from $M$ can be written uniquely as a sum
	of an element from $M_1$ and $M_2$.
\end{definition}
As another example, we have that the $xy$-plane consists
of sums of its vectors.
\begin{example}[Euclidean Plane]
	Take the vector space $\RR^2 = \left\{ (x,y) \mid x \in \RR, y \in \RR \right\}$.
	We can consider it as a direct sum of its two $x$-axis and $y$-axis:
...
for \[ V = \left\{ ax^2+bx+c \mid a,b,c \in \RR \right\} \]
...
\end{definition}
\begin{definition}
	Consider a finite\footnote{
		You can have infinite sets as well; for example,
		you probably want to think of the set of all polynomials
		as having an infinite basis $\left\{ 1,x,x^2,x^3,\dots \right\}$.
		The catch is that if your set is infinite,
		your linear combinations must have at most finitely many
		nonzero terms.
		In the polynomial example, that just means
		that your polynomials have finite degree (which is good).
	}
	set of $v_1, \dots, v_n$ of vectors in a module $M$.
	\begin{itemize}
		...
		\ii It is called \vocab{generating set} if every $v \in M$ can be written as
...
\begin{figure}[ht]
	\centering
	\includegraphics[height=7cm]{/home/evan/Pictures/TopologicalGG/basis-span-fantasy.jpg}
...
\begin{theorem}[Maximality and Minimality of Bases]
	If $e_1$, $e_2$, \dots, $e_n$ are a basis of a module $M$, then
	\begin{enumerate}[(a)]
		\ii The $\{e_i\}$ are spanning, but no proper subset are.
		\ii The $\{e_i\}$ are linearly independent, but adding any other
		element of $M$ makes them not linearly independent.
	\end{enumerate}
	If $M$ is a vector space, conditions (a) and (b) also imply that the $\{e_i\}$ are a basis.
...
\begin{proof}
	...
	hence $M$ is required to be a vector space instead of just a module.
\end{proof}
...
\end{theorem}
\begin{proof}
	We prove something even stronger:
...
\end{proof}
\begin{remark}
	In fact, this is true for modules over any ring.
	(Well, you need the ring to be commutative, but in this book all rings are commutative).
...
a vector space to be the size of any finite basis, if one exists.
...
\section{Linear Maps}
We've seen homomorphism and continuous maps.
Now we're about to see linear map, the structure preserving maps
...
\begin{definition}
	...
	\end{enumerate}
	If this map is a bijection, it is an \vocab{isomorphism}.
	We say $V$ and $W$ are \vocab{isomorphic} vector spaces
	and write $V \simeq W$.
\end{definition}
...
Moreover, although the space of quadratic polynomials
does have a natural basis,
a lot of spaces don't have such an obvious choice;
...
So rather than you telling you the value of $T(e_1)$ in some abstract space $W$,
...
is the same thing we get from interpreting the function $S \circ T$ as a matrix.
...
\begin{theorem}[Basis Completion]
	...
	\begin{enumerate}[(a)]
		...
		\ii If $e_1, \dots, e_m$ is a basis of $V'$, then there exists
...
\begin{proof}[Sketch of Proof]
	...
	You can show that $T$ is injective over the span of $v_1, \dots, v_k$,
	so its images in $W$ are linearly independent.
...
in particular, you can write for $T : V \to W$, 
one can write $V = \ker T \oplus V'$, so that $T$ annihilates its kernel while sending $V'$
...
\begin{theorem}
	...
	Then there exists a \emph{unique} polynomial of degree $P$
	at most $n$ passing such that \[ P(x_i) = y_i \]
	for every $i$.
\end{theorem}
\begin{proof}
...
that the only vector space in existence is $\RR^{\oplus n}$.
...
I focused mostly on vector fields (aka modules over a ring) in this chapter
for simplicity, so I want to make a few remarks about modules over a general ring $R$ before concluding.

Firstly, recall that for general modules, we say ``generating set'' instead of ``spanning set''.
Shrug.

The main issue with rings is that our key theorem \Cref{thm:vector_best} fails in spectacular ways.
For example, consider the $\ZZ$ as a $\ZZ$-module over itself.
...
\begin{definition}
	A $R$-module $M$ is called \vocab{finitely generated} if it as a finite generating set.
\end{definition}
\begin{definition}
...
(mainly in the chapter on algebraic number theory);
we'll even use the notation $G \oplus H$ for two abelian groups $G$ and $H$
...
\section{A Quick Word on Dot Products}
Let's throw orthogonality into the mix.
To do this we want a notion of a ``dot product'' between two vectors.
This is provided by a so-called inner form.

I'll just do the case where $k = \RR$.

\begin{definition}
	Let $V$ be a real vector space.
	An \vocab{inner form} $\left< \bullet, \bullet \right> : V \times V \to \RR$ is a map with the following properties.
	\begin{itemize}
		\ii The form is \textbf{linear in the first argument}, so 
		$\left<a+b,c\right> = \left<a,c\right>+\left<b,c\right>$
		and $\left<r \cdot a,b\right> = r \cdot \left<a,b\right>$.
		\ii The form is symmetric: $\left<a,b\right> = \left<b,a\right>$.
		This implies that it is also linear in the second argument.
		\ii The form is \textbf{positive definite}, meaning $\left<v,v\right> \ge 0$ is a real number, and equality takes place only if $v = 0_V$.
	\end{itemize}
\end{definition}

\begin{example}
	If $V = \RR^n$, then one inner form is the standard dot product.
	Specifically, if $v = \left( a_1, \dots, a_n \right) \in \RR^n$
	and $w = \left( b_1, \dots, b_n \right) \in \RR^n$ then
	\[ \left<v,w\right> \defeq a_1b_1 + \dots + a_nb_n \]
	works.
\end{example}

\begin{definition}
	The \vocab{norm} of a $v \in V$ is defined by $\norm{v} = \left<v,v\right>$.
	Evidently this is positive for all nonzero $v$,
	since the inner form is positive definite.
\end{definition}

\begin{definition}
	Let $v_1, \dots, v_n \in V$.
	We say that $v_i$ are \textbf{orthogonal} if $\left< v_i, v_j \right> = 0$ for all $i \neq j$.
	Moreover, it is \textbf{orthonormal} if $\norm{v_i} = 1$ for all $i$.
\end{definition}
\begin{example}
	Let $V = \RR^n$ and use the standard dot product as the inner form.
	Then the standard basis $e_1 = \left( 1,0,\dots,0 \right)$, $e_2 = (0,1,\dots,0)$, et cetera is an orthonormal basis.
\end{example}

As you might expect, orthogonal vectors should be linearly independent,
just based on our geometric intuition for $\RR^n$.
\begin{lemma}
	Any orthonormal set of vectors is linearly independent.
\end{lemma}
\begin{proof}
	Suppose $\sum a_iv_i = 0_V$.
	Then \[ 0_V = \left< v_1, \sum a_i v_i \right> = a_1 \norm{v_i} = a_1. \]
	Hence $a_1 = 0$.
	Similarly $a_2 = \dots = a_m = 0$.
\end{proof}

Finally, we can show that we can select orthogonal bases.
...
\begin{proof}
	...
	Inductively, construct $e_i$ such that all $e_i$ are orthonormal
	and the span of $e_1, \dots, e_k$ equals the span of $v_1, \dots, v_k$.
	Eventually we end up with $n$ orthonormal $e_i$'s.
	They are linearly independent, but there are $n$ of them,
	so they must form a basis.
\end{proof}
