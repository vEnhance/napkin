...
\begin{dproblem}
	...
	\begin{enumerate}[(a)]
	\ii Show that $M^\dagger M$ and $M$ have equal rank.
	\ii Show that $M^\top M$ and $M$ need not have equal rank.
	\end{enumerate}
	\begin{hint}
		For the first part, show that $\ker M^\dagger M = \ker M$
		(not just the dimension, they are actually the same kernel).
		For the second part, try to construct a nonzero $2 \times 2$ complex matrix $M$
