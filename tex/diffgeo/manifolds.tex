...
\begin{definition}
	...
	\[
		\phi_{ij} : E_i \cap \phi_i``(U_i \cap U_j)
		\taking{\phi_i\inv}
		U_i \cap U_j
		\taking{\phi_j} E_j \cap \phi_j``(U_i \cap U_j).
	\]
\end{definition}
Sorry for the dense notation, let me explain.
The intersection with the image $\phi_i``(U_i \cap U_j)$
and the image $\phi_j``(U_i \cap U_j)$ is a notational annoyance
