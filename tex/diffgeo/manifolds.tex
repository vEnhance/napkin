...
dual space of $\mm_p / \mm_p^2$;
for this reason, the space $\mm_p / \mm_p^2$ is defined as the
...
\begin{theorem}
	...
	\[
	\begin{aligned}
		\mm_0 &= \{ \text{smooth functions } f : f(0) = 0 \} \\
		\mm_0^2 &= \{ \text{smooth functions } f : f(0) = 0, (\nabla f)_0 = 0 \}.
	\end{aligned}
	\]
	In other words $\mm_0^2$ is the set of functions which vanish at $0$
	and such that all first derivatives of $f$ vanish at zero.
\end{theorem}
Thus, it follows that there is an isomorphism
\[ \mm_0 / \mm_0^2 \cong \RR^n
	\quad\text{by}\quad
	f \mapsto
