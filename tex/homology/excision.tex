...
This is already enough to deduce:
\begin{lemma}
	[Homology Relative to Contractible Spaces]
	\label{lem:rel_contractible}
	Let $X$ be a topological space,
	and let $A$ be a contractible subspace of $X$.
	Then for all $n$, \[ H_n(X, A) \cong \wt H_n(X). \]
\end{lemma}
\begin{proof}
...
In particular, if $A$ is a point we obtain that
\begin{moral}
	$H_n(X, \{\ast\})$ is just the reduced homology group $\wt H_n(X)$.
\end{moral}

Finally, by \Cref{prob:triple_long_exact}
we even have a long exact sequence
\[
	\dots
	\to H_n(B,A)
	\to H_n(X,A)
...
\begin{problem}
	...
	Show that for every integer $n$, $H_n(X) \cong H_n(Y)$.
\end{problem}
...
%\begin{problem}
%	A topological $n$-manifold $X$ is a (Hausdorff) space such that for
%	every point $p \in X$, some neighborhood of $p$ is homeomorphic
%	to an open set of $\RR^n$.
%	What are the local homology groups of an $n$-manifold?
%\end{problem}
