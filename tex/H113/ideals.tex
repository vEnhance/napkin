...
\begin{center}
	\begin{tabular}[h]{lcc}
		...
		Sub-structure & subgroup & (not discussed)\footnote{You
		can guess the definition a ``subring'' but we will not use it.} \\
...
\begin{abuse}
	...
	$\ZZ[i]$ should officially be ``integer-coefficient polynomials in $i$''.
	However, as $i^2=-1$, a polynomial in $i$ is just the same as
...
we did with quotient groups.\footnote{%
...
So this is stronger than just saying $I$ is closed under multiplication.
\begin{remark}
...
\begin{proposition}
	...
	Then $I$ is proper if and only if it contains no units of $R$.
\end{proposition}
\begin{ques}
	Prove this.
\end{ques}
...
recognize that this as just $5\ZZ = (5)$.
...
\begin{example}
	[Examples of PID's]
	As just said, for now you should just verify
...
\begin{sproblem}
	\label{prob:inclusion_reversing}
	Let $R$ be a ring and $I$ an ideal.
	Find an inclusion reversing bijection between
	\begin{itemize}
