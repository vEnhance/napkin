...
\begin{example}[Polynomial ring]
	...
	\[ R[x] = \left\{ a_n x^n+a_{n-1}x^{n-1}+\dots+a_0 \mid a_0, \dots, a_n \in R \right\}. \]
...
\prototype{$\ZZ$ is a PID, $\ZZ[x]$ is not but is
at least Noetherian.
$\ZZ[x_1, x_2, \dots]$ is not Noetherian.}
...
\begin{example}[Examples of prime ideals]
	...
	\begin{enumerate}[(a)]
		...
		is \emph{not} prime, since the elements
...
\begin{ques}
	Conclude that maximal ideals are prime.
\end{ques}
