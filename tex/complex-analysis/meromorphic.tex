...
By the Identity Theorem, there are cannot be too many zeros of $B$!
...
We will then extend our results on contours to such situations.
...
\begin{definition}
	A function $f : U \to \CC$ is \vocab{meromorphic} if it can be expressed as $A/B$
	for $A, B : U \to \CC$ holomorphic, with $B$ not identically zero in any neighborhood.
\end{definition}
Let's see how this function $f$ behaves like.
...
with $c_k \neq 0$ (certainly coefficient is nonzero since $B$ is not identically zero!).
Then we can write
\[ \frac{1}{B(z)} = \frac{1}{z^k} \cdot \frac{1}{c_k + c_{k+1}z + \dots}. \]
But the fraction on the right is a holomorphic function is this neighborhood!
...
\begin{definition}
	...
	For each $p \in S$, called a \vocab{pole} of $f$, the function $f$ must admit a \vocab{Laurent series},
	meaning that
	\[
		f(z) =
		\frac{c_{-m}}{(z-p)^m}
...
\begin{example}
	...
	\begin{align*}
		...
		&= \frac 1z \cdot \frac{z+1}{1 - \left( \frac{z^2}{3!} - \frac{z^4}{5!} + \frac{z^6}{7!} \right)} \\
...
\begin{example}
	...
	\end{align*}
	It has a pole of order $1$ and residue $2$ at $z=0$.
