...
\begin{example}
	...
	\begin{enumerate}[(a)]
		\ii The \vocab{Devil's Staircase} (or Cantor function) is a continuous function $H : [0,1] \to [0,1]$
...
and that knowing tiny amounts of data about the function can deterimne all its values.
%In particular, they are highly \emph{rigid} and \emph{regular}.
Some highlights:
\begin{itemize}
	\ii It'll turn out that having a first derivative is enough to guarantee having \emph{all} derivatives
	(which is unlike the real case, in which no number of derivatives will guarantee the rest).
	\ii Now that you have all derivatives, you can take the Taylor series, (unlike the real case,
	in which even if you have all derivatives it's possible the Taylor series is totally wrong).
	\ii It'll turn out that knowing just the values of a holomorphic function on the boundary
	of the unit circle will tell you all the values everywhere.
	\ii Even knowing just the values of the function at $1$, $\half$, $\frac13$, \dots
	is enough to determine the whole function!
	\ii Integrals in the complex plane will just work like magic.
\end{itemize}
...
Then for some $z \in U$, we define the \vocab{derivative} at $z_0$ to be
\[
...
\begin{example}
	...
	Complex conjugation, $z \mapsto \ol z$, is an example
	of a map which is not differentiable at zero.
	As we approach $0$ from various directions, we get different
	values of the ratio $\frac{f(h)}{h}$ vary:
	\begin{center}
		\includegraphics[width=4cm]{media/complex-conj-not-holo.png}
	\end{center}
\end{example}
...
\begin{proof}
	...
	\begin{center}
		...
		\end{asy}
	\end{center}
	We're going to do the following trick: construct a \vocab{keyhole contour} $\Gamma_{\delta, \eps}$
...
\prototype{Imagine a formal series!}
...
\begin{align*}
	...
	\intertext{You can prove (using the so-called Weirestrass M-test) that the summation order can be switched:}
...
\begin{theorem}
	[Cauchy's Differentiation Fomula]
...
that a function being complex differentiable once means it's not only infinitely differentiable,
...
It's more subtle in the case if $U$ is, for example, a square; you can't cover the entire square
...
\begin{figure}[ht]
	\centering
	\includegraphics[height=6cm]{/home/evan/Pictures/TopologicalGG/bounded-like-complex-sine.jpg}
...
\begin{sproblem}
	[Identity Theorem]
	\gim
	Let $f, g : U \to \CC$ be holomorphic.
	\begin{enumerate}[(a)]
		\ii Suppose $z_1$, $z_2$, \dots is a sequence of \emph{distinct} complex numbers in $U$ which
		converges to a point $z \in U$.  Given that $f(z_i) = g(z_i)$ for all $i$,
		prove that $f = g$ on some open disk centered at $z$.
		\ii Assume $U$ is \emph{connected}\footnote{This means $U$ has no proper clopen subsets;
		hence the right approach to this problem is to show the set of agreement is clopen.
		For example, this means that an entire function is determined by its values
		by the values $f(\frac11), f(\frac12), \dots$, or on any open neighborhood!
		}.
		Show that $f = g$ on all of $U$.
	\end{enumerate}
	\begin{hint}
		For (a), look at the Taylor series of $f-g$.
		For (b), take the interior of the agreeing points.
	\end{hint}
	\begin{sol}
		For part (a), WLOG set $z=0$.
		Look at the Taylor series of $f - g$ around $z=0$.
		If it isn't zero, write it as $a_Nz^N + a_{N+1}z^{N+1} + \dots$, $a_N \neq 0$.
		But by continuity of $h(z) = a_N + a_{N+1}z + \dots$ there is some
		neighborhood of zero where $h(z) \neq 0$.

		For part (b), let $S$ be the interior of the points satisfying $f=g$.
		By definition $S$ is open.
		Use (a) to show $S$ is closed: if $z_i \to z$ and $z_i \in S$,
...
\begin{remark}
	...
	If $f : \CC \to \CC$ is entire and nonconstant,
	then the image of $f$ is either $\CC$ or $\CC$ minus a point.
	This is Little Picard's Theorem, proved in a later\todo{yet to be written} chapter.
	But the proof uses Liouville's Theorem as an intermediate step.
\end{remark}
