...
\begin{definition}
	...
	denoted $A_f$, to be $\{1, f, f^2, f^3, \dots\}\inv A$.
	(Note that $\left\{ 1, f, f^2, \dots \right\}$ is multiplicatively closed.)
\end{definition}
...
\end{definition}
\begin{ques}
	Why is $S = A \setminus \kp$ multiplicatively closed
...
\section{Prime ideals of localizations (and quotients)}
...
\begin{corollary}
	...
	\begin{enumerate}[(a)]
		...
		then the prime ideals of $A_f$ are naturally
...
\begin{example}
	[Prime ideals of $A_6$]
	Let $A = \ZZ$ and
	suppose we localize away from the element $6$,
	i.e.\ consider $A_{6}$.
	As we saw,
	\[ A_{6} = \left\{ \frac{n}{2^x 3^y} \mid n \in \ZZ,
		x,y \in \ZZ_{\ge 0} \right\}.  \]
	consist of those rational numbers whose
	denominators have only powers of $2$ and $3$.
	Note that $(5) \subset A_6$ is a prime ideal:
	those elements of $A_6$ with $5$ dividing the numerator.
	Similarly, $(7)$, $(11)$, $(13)$, \dots\
	and even $(0)$ give prime ideals of $A_6$.
...
\begin{example}
	...
	Let $A = \ZZ$ and
	suppose we localize at the prime $(5)$,
	i.e.\ consider $A_{(5)}$.
	As we saw,
	\[ A_{5} = \left\{ \frac{m}{n} \mid m,n \in \ZZ,
			5 \nmid n \right\}.  \]
	consist of those rational numbers whose
...
While we are here, we mention that quotients
...
\begin{proposition}
	...
	then the prime ideals of $A/I$
	are in bijection with ideals of $A$
	which are \textbf{supersets of} $I$.
\end{proposition}
\begin{proof}
	Consider the quotient homomorphism $\psi \colon A \to A/I$.
	For any prime ideal $\kq \subseteq A/I$,
	its pre-image $\iota\pre(\kq)$ is a prime ideal
	(by \Cref{prob:prime_premimage}).
...
\begin{problem}
	...
	Compute $A_{60}$, the localization of $A$ away from $60$.
	\begin{sol}
		One should get $A_{60} = \Zc{7}$.
	\end{sol}
\end{problem}
