...
\begin{exercise}
	Prove that the equivalence relation is precisely that if we define $a_1/s_1 = a_2/s_2$ to be equal if $a_1s_2-a_2s_1=0$, then take the transitive closure.
\end{exercise}
...
\begin{example}
	...
	\begin{enumerate}[(a)]
		...
		% TODO must be Laurent series? But I think not all, only count those convergent in a positive radius around center?
...
%Comparing with the example of localizing away from an element above, you can see that
...
%Thus we see why we say the 2 notations opposite.
%
%Thinking of functions that "may not blow up at the whole vanishing set" can be confusing, so
%another (hopefully) more intuitive way to think about localizing at a prime is that the function
%\textbf{must not blow up at the point corresponding to the prime ideal}.
%
%For example, if 𝔭 is the ideal (y-x²), then A_𝔭 is the set of functions that does not blow up at the
