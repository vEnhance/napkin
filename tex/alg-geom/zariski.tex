...
\begin{example}[Zariski topology on $\Aff^2$]
	...
	(possibly empty) of
	\begin{itemize}
		\ii Closed curves, like $\VV(y-x^2)$ (which is a parabola), and
...
\end{example}
\begin{ques}
	Show that every open set of a $V \subseteq \Aff^n$
...
\begin{definition}
	...
	every point $p$, we can find an open set $U_p \subseteq U$ containing $p$
