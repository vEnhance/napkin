...
as the times allocated for $\gamma_1$, $\gamma_2$, $\gamma_3$.
\begin{ques}
...
so paths $\gamma$ lives in some ``homotype type'',
...
\begin{example}[Loops in $S^2$ are nulhomotopic]
	...
	\begin{center}
		\includegraphics[width=12cm]{media/S2-simply-connected.jpg}
		% COPYRIGHT ISSUE
	\end{center}
\end{example}
...
It's easy to show that if $\gamma_1 \simeq \gamma_2$ are path-homotopic,
then in fact
\[ f_\sharp \gamma_1 \simeq f_\sharp \gamma_2. \]
(Proof: just take the homotopy $h$ taking $\gamma_1$ to $\gamma_2$
...
\begin{remark}
	...
	of the covariant Yoneda functor (\Cref{def:covariant_yoneda}).
\end{remark}
...
Two functions $f, g : Y \to X$ if there exists a continuous
...
This homotopy can be quite dramatic, as the following example shows.\todo{
...
\begin{definition}
	...
	They are \vocab{homotopy equivalent} if there exists
...
\begin{example}[$\CC$ is homotopy equivalent to a point]
	Consider the topological spaces $\CC$ and the space consisting of the single point $0$.  We claim these spaces are homotopy equivalent (can you guess what $f$ and $g$ are?)
	Indeed, the two things to check are
	\begin{enumerate}[(i)]
		\ii $\CC \to \{0\} \hookrightarrow \CC$ by $z \mapsto 0 \mapsto 0$ is homotopy equivalent to the identity on $\CC$, which we just saw, and
...
\end{example}
\begin{remark}
	$\CC$ cannot be homeomorphic to a point because one cannot even biject the sets correctly.
\end{remark}

\begin{example}[$\CC \setminus \{0\}$ is homotopy equivalent to $S^1$]
	Consider the topological spaces $\CC \setminus \{0\}$, the \vocab{punctured plane}, and the circle $S^1$ viewed as a subset of $S^1$.
...
\end{example}
\begin{remark}
	$\CC \setminus \{0\}$ cannot be homeomorphic to $S^1$
...
\end{proof}
As we will see, the homology groups are also helpless.
...
\begin{remark}
	There is a categorical way to interpret all of this.
	One can construct a category $\catname{hTop}_\ast$ whose \emph{objects} are pairs $(X,x_0)$
	of spaces with a distinguished basepoint, but whose arrows are the \emph{homotopy classes}
	of continuous functions (rather than just the functions themselves).
	Then what we've just seen is that there is a functor
	\[ \catname{hTop}_\ast \to \catname{Gp} \]
	sending each $(X, x_0)$ to $\pi_1(X, x_0)$.
	In fact, $\pi_1(X, x_0)$ is the set of arrows $(S^1, 1) \to (X, x_0)$ in $\catname{hTop}_\ast$,
	so this looks exactly like the covariant Yoneda functor again,
	except with target $\catname{Gp}$ instead of $\catname{Set}$.

	In any case the theorem above is then killed by the fact
	that functors preserve isomorphism (\Cref{thm:functor_isom}).
	You can see this from the proof: we've done the same trick of looking at $f \circ g$ and $g \circ f$ again.
\end{remark}
...
\begin{problem}[Harmonic Fan]
	...
	path-connected but for which a point $p$ can be found such
	any $r$-neighborhood $p$ with $r < \half$ is not path-connected.
	% harmonic fan
\end{problem}

\begin{dproblem}
	[Seifert-van Kampen] \gim
	Let $X$ be a topological space.
	Suppose $U$ and $V$ are open subsets such that
	\begin{itemize}
		\ii Both $U$ and $V$ are simply connected (as subspaces).
		\ii As sets, $X = U \cup V$.
		\ii The intersection $U \cap V$ is nonempty and path-connected.
	\end{itemize}
