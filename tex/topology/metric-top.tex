...
\end{example}
\begin{example}
	[Function Space] We can let $M$ be the space of integrable functions $[0,1] \to \RR$ and define the metric by $d(f,g) = \int_0^1 \left\lvert f-g \right\rvert \; dx$.
\end{example}
...
\begin{example}
	...
	\[
		...
		\end{cases}
	\]
	If $\left\lvert S \right\rvert = 4$ you might think of this space as the vertices of an regular tetrahedron, living in $\RR^3$.
...
we use both the metric of $M$ for one condition and the metric of $N$ on the other condition.
...
The next example illustrates why this criteria can often be much easier to work with.
\begin{proposition}
...
You might be surprised that we need the condition $f\inv$ is also continuous.
The reason we didn't need it when we considered group isomorphisms
you can show that if $\phi$ is a homomorphism of groups which is a bijection,
then so is $\phi\inv$.
But the same is not true for homeomorphisms.
\begin{exercise}
...
\begin{example}[Examples of Homeomorphisms]
	...
	\begin{enumerate}[(a)]
		...
		\ii A doughnut is homeomorphic to the coffee cup.
...
\begin{abuse}
	Just to be perfect clearly: by a ``open neighborhood'' I mean \emph{any} open set containing $x$.
	But by an ``$r$-neighborhood'' I always mean the points with distance less than $r$ from $x$,
