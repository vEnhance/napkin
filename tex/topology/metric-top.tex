...
\begin{theorem}
	A function $f \colon M \to N$ of metric spaces is a homeomorphism if
	\begin{enumerate}[(i)]
...
\begin{itemize}
	...
	For a sequence to converge one has to be able to
	write down where it approaches,
	but $\sqrt 2 \notin \QQ$.
	...
	``does $(x_n)$ converge''
	without specifying which space one is discussing.

	The fact that these sequences fail to converge
	even though they ``ought to'' is weird and bad,
	and so in the next section we'll define
	\emph{complete} metric space as one where sequences
...
We close this chapter by talking about a way to fix the issue
of ``sequences that should converge but don't..
...
For example, the completion of $\QQ$ is $\RR$
...
\begin{dproblem}[Completion of a metric space]
	...
	Let $M$ be a metric space.
	Construct a metric space $\ol M$ such that
	\begin{itemize}
		\ii $\ol M$ is complete,
		\ii there is an injective map $M \injto \ol M$
		(so one can consider $M$ as a subspace of $\ol M$), and
		\ii every open set of $\ol M$ contains a point of $M$
