...
\begin{center}
	\begin{asy}
		...
		filldraw(ellipse((0, 3.2), a, b), white, linewidth(1.2));
		label("$\vdots$", (0, 2.64), fontsize(14));
		filldraw(ellipse((0, 1.6), a, b), white, linewidth(1.2));
		filldraw(ellipse((0, 1.95), a, b), white, linewidth(1.2));

		path bracePath = shift((-1.34, 1.6)) * rotate(90) * brace(0, 1.58, 0.26);
		draw(bracePath, linewidth(1));
		label("$p^{-1}(U)$", (-2.08, 2.4), fontsize(14));

		draw((0, 1.02)--(0, 0.28), linewidth(1), Arrow(size=4.8));
		label("$p$", (0.02, 0.7), E, fontsize(14));

		draw(ellipse((0, -0.3), a, b), linewidth(1.2));
		label("$U$", (-1.84, -0.3), fontsize(14));
	\end{asy}
\end{center}
...
\end{tikzcd}
\end{center}
where $Y$ is a sufficiently nice space, as follows.
\begin{definition}
	A space $Y$ is \vocab{locally path-connected} if, at any point $x\in Y$,
	every open neighborhood $U$ of $x$ contains a path-connected open neighborhood
	$V\subseteq U$.
\end{definition}
\begin{theorem}[General lifting criterion]
	\label{thm:lifting}
	Let $p \colon (E, e_0) \to (B, b_0)$ be a covering projection. If $Y$ is a
	path-connected and locally path-connected space, then for any continuous
	$f \colon (Y,y_0) \to (B, b_0)$, a lifting $\tilde f$ with $\tilde f(y_0) = e_0$
...
\end{remark}
\begin{remark}[The locally path-connected condition really is necessary]
	Let $Y$ be the \textit{Warsaw circle}, depicted below,
	consisting of the graph of $y=\sin(1/x)$ for $0<x<1/\pi$,
	the segment $\{0\}\times[-1,1]$, and a curve to close everything up.
	\begin{center}
		\begin{asy}
			size(4cm);
			import graph;
			...
			real w = 0.55;  // width
			real h = 0.22;  // amplitude
			real a = 0.70;  // horizontal radius
			real b = 0.55;  // vertical radius

			real k = 25;
			real f(real x) {
			    return h * sin(k * log(w/x));
			}

			draw((0, -h) -- (0, h), linewidth(2.5) + squarecap);
			...
			for(int i = n; i >= 0; --i) {
			    real t = i / (real)n;
			    real x = w * exp(-t * max_depth);
			    sine_curve = sine_curve -- (x, f(x));
			}

			real cx = w / 2;
			real theta0 = acos((w / 2) / a);
			real cy = -b * sin(theta0);

			guide loop;
			int steps = 500;
			real sweep_angle = pi + 2 * theta0;
			for(int i = 0; i <= steps; ++i) {
    			real theta = theta0 - sweep_angle * (i / (real)steps);
    			real px = cx + a * cos(theta);
    			real py = cy + b * sin(theta);
    			loop = loop -- (px, py);
			}

			draw(sine_curve -- loop, linewidth(2.5) + roundjoin);
		\end{asy}
    \end{center}
	...
	because $Y$ is not locally path-connected at the origin.
\end{remark}
