...
It is possible for a set to be both open and closed (or \vocab{clopen});
for example $\varnothing$ and the entire space are examples of clopen sets.
In fact, the presence of a nontrivial open set other than these two leads
...
See also \Cref{prob:local_path_connected}.

\todo{topologist's sine curve}
...
\begin{definition}
	...
	If a path homotopy exists, we say $\alpha$ and $\beta$ are path \vocab{homotopic} and write $\alpha \simeq \beta$.
\end{definition}
\begin{abuse}
...
You might have noticed that the open sets of $\RR$ are a little annoying to describe: the prototypical example of an open set is $(0,1)$,
...
The reason you can do this is that for a metric, every subset $S \subseteq M$ is a topological space in its own right (just use the same distance function). This is called the \vocab{subspace topology}.
...
\begin{dproblem}
	...
	Show that there exists a nonconstant function $X \to \{0,1\}$ if and
...
\begin{problem}
	...
	Prove that there such a disjoint union has at most \emph{countably many}
	intervals in it.
	\begin{hint}
		Appeal to $\QQ$.
	\end{hint}
