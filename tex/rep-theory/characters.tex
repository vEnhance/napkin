...
\begin{ques}
	...
	then $\chi(g) = \chi(h)$.
\end{ques}
Now, given our algebra $A$ we define the \vocab{center} $Z(A)$
to be the set of elements $x \in A$ for which $ax = xa$ for all $a \in A$.
So we can think of each character $\chi_V$ as an element of $Z(A)^\vee$.

\begin{example}
	[Examples of centers]
	\listhack
	\begin{enumerate}[(a)]
		\ii If $A$ is commutative, then $Z(A) = A$.
		\ii If $A = \Mat_k(d)$, then $Z(A)$ is one-dimensional;
		it's the set of scalar matrices.
		\ii Suppose $A = k[G]$.  We claim that $\dim Z(A)$ is equal to the
		number of conjugacy classes of $A$.
		Indeed, an element of $A$ can be thought of as just
		an arbitrary function $\xi \colon G \to k$.
		So an element of $Z(A)$ is a function $\xi \colon G \to k$ such that
...
\begin{theorem}
	...
	\begin{enumerate}[(a)]
		...
		linearly independent as elements of $Z(A)^\vee$.
		\ii If $A$ is finite-dimensional and semisimple,
		then the characters attached to irreps
		form a basis of $Z(A)^\vee$.
	\end{enumerate}
	In particular, in (b) the number of irreps of $A$ equals $\dim Z(A)$.
\end{theorem}
\begin{proof}
	...
	we need only show that $\dim Z(A) = r$,
	which follows from the observation earlier that each $\Mat(V_i)$
	has a one-dimensional center.
\end{proof}
Since $G$ has $\dim Z(\CC[G])$ conjugacy classes,
