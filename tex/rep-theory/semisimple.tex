...
we already know that if $V$ and $W$ are finite-dimensional irreps, then
\[
	\Homrep(V,W) \cong
	\begin{cases}
		k & \text{if $V \cong W$} \\
...
More generally, suppose $V$ is a finite-dimensional irrep and consider
...
\begin{theorem}
	...
	Let $V$ and $W$ be finite-dimensional completely reducible representations,
...
finite-dimensional algebras have finitely many finite-dimensional irreps.
...
\begin{moral}
	...
	for finite-dimensional irreps of $A$.
\end{moral}
...
Similarly, given lots of different nonisomorphic finite-dimensional irreps of $A$,
...
\begin{corollary}
	...
	Any finite-dimensional algebra $A$ has at most $\dim A$
...
\begin{theorem}
	...
	where the $V_i$ are the finite-dimensional irreps of $A$;
...
\begin{remark}
	...
	it's the set of all $a \in A$ which act by zero in all finite-dimensional irreps of $A$.
