...
\begin{lemma}
	...
	Let $X = (X, \in)$ be a model, where $X$ is a set (possibly not transitive).
	Then there exists an isomorphism $\pi : X \to M$ for
...
\begin{proof}
	...
	Since $\in$ is well-founded and extensional, we can look at the
	$\in$-minimal element $x_\varnothing$ of $X$ with respect to $\in$.
...
\begin{proof}
	...
	First, since $\ZFC^+$ we can take $M = V_\kappa$ as a \emph{set}
