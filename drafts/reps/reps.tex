...
We compute all irreducible representations of $\CC[S_3]$.
...
we'll in fact see that the number of representations of $k[G]$
...
\begin{itemize}
	\ii There is a trivial one-dimensional representation $\CC_{\text{triv}}$
	where the map $S_3 \to \CC^\times$ is the identity.
	Thus in $\CC_{\text{triv}}$ every $\sigma \in S_3$ acts as the identity.

	\ii There is a nontrivial one-dimensional representation $\CC_{\text{sign}}$
	where the map $S_3 \to \CC^\times$ is given by sending $\sigma$
	to the sign of $\sigma$.
	Thus in $\CC_{\text{sign}}$ every $\sigma \in S_3$ acts as $\pm 1$.
	\begin{exercise}
		Check that $\CC_{\text{sign}}$ are $\CC_{\text{triv}}$
		are not isomorphic.
	\end{exercise}
...
\end{itemize}
This implies that these are all the irreducible representations of $S_3$.
Note that, if we take the representation $V$ of $S_3$ on $k^{\oplus 3}$,
we just get that $V = \refl_0 \oplus \CC_{\text{triv}}$.

\section\problemhead
\todo{problems like $D_{2n}$}

\chapter{Definition of A Character}
...
We are especially interested in the case $A = k[G]$.
...
\begin{example}
	...
	and compute the trace of each $\sigma$ directly in this basis.
	\[
		\begin{array}{|r|rrr|}
			\hline
...
all of which are true more generally.
\begin{itemize}
	\ii It seems that $\chi_V(g)$ only depends on
	the conjugacy class of $g$.
	\ii The sum of the squares are any column is $6$ again!
	\ii The ``dot product'' of any two columns is zero.
	\ii The number of columns equals the number
\end{itemize}
We now set about to explain this behavior for any finite group $G$.

\begin{abuse}
	The name ``character'' for $\chi_V : G \to k$ is a bit of a misnomer.
	This $\chi_V$ is not additive or multiplicative in any way,
...
\[ \chi_V(ab) = \chi_V(ba). \]
For this reason we say $\chi_V$ is a \vocab{class function}.
