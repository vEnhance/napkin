...
\begin{example}
	...
	but we can also do this geometrically by computing the
...
\begin{dproblem}
	[Reading Decompsitions from Characters]
...
\begin{problem}
	...
	is $9$-dimensional, so it is clearly irreducible.
...
\begin{example}[Frobenius Determinants]
	...
	\begin{pmatrix} x_{\id} & x_T \\ x_T & x_{\id} \end{pmatrix}. \]
...
\begin{proof}
	...
	\[
		\det T
		= \prod_{i=1}^r \left( \det T|_{V_i}  \right)^{\dim V_i}.
	\]
	where $T|_{V_i}$ is the restricted map $T : V_i \to V_i$.
	...
	\begin{itemize}
		\ii \emph{Irreducible}:
		By Density, for any $M \in \Mat(V_i)$ there exists
		...
		Thus the ``generic'' $T|_{V_i}(\{x_g\})$, viewed as a matrix with
