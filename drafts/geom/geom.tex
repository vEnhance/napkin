...
until we have all the data of a ringed space; in the next chapter, we'll then create general varieties.
The ideas here will later grow up to become the core of modern algebraic geometry: the \emph{scheme}.
...
\begin{definition}
	...
	the \emph{closed sets} are those of the form $\VV(I)$;
	i.e.\ the vanishing loci of some ideal $I \subseteq \CC[x_1, \dots, x_n]$.
...
We want to define a notion of $\OO_V(U)$ for any open set $U$: the ``nice'' functions on any open subset.
Obviously, any function is the $\CC[V]$ will work as a function on $\OO_V(U)$.
However, to capture more of the structure we want to loosen our definition slightly of ``nice'' function slightly
by allowing \emph{rational} functions.

The chief example is that $1/x$ should be a regular function on $\Aff^1 \setminus \{0\}$.
However, the definition is slightly more convoluted than that: we have to allow for multiple representations
in the following way:
\begin{definition}
	Let $U \subseteq V$ be open.
	We say a function $\phi : U \to \CC$ is \vocab{regular} if
	for every point $p$, we can find an open set $U_p$ containing $p$ and polynomials $g_p, h_p \in \CC[V]$ such that
	\[ \phi(x) = \frac{g_p(x)}{h_p(x)} \qquad \forall x \in U_p. \]
	In particular, $h_p(x) \neq 0$ on $U_p$.
	We denote the set of all regular functions by $\OO_V(U)$.
\end{definition}

%\begin{abuse}
%	There is a tiny abuse of notation here:
%	I defined the coordinate ring $R$ last section
%	and then define $\OO_V(U)$ for any open subset $U \subseteq V$ in terms of $R$.
%	So $\OO_V(V)$ could refer to two things:
%	the original coordinate ring $R$ of polynomial functions,
%	or the regular functions for $U = V$.
%	Fortunately, we'll see in a moment that these coincide.
%\end{abuse}

This definition is misleading complicated, and the examples should illuminate it significantly.
Firstly, in practice, most of the time we will be able to find a ``global'' representation
of a regular function as a quotient, and we will not need to fuss with the $p$'s.
For example:
\begin{example}
	[Regular Functions]
	\listhack
	...
	\end{enumerate}
\end{example}
The ``local'' clause with $p$'s is just to avoid some strange edge cases.
\begin{example}
	...
	\[
		...
		\end{cases}
	\]
	Clearly these are the ``same function'' (since $ab=cd$), but we cannot write ``$a/d$'' or ``$c/b$''
...
The division-by-zero, as one would expect, essentially prohibits regular functions on the entire space $V$;
i.e.\ there are no regular functions in $\OO_V(V)$ that were not already in $\CC[V]$.
Actually, we have the following more general result which computes the regular functions on distinguished open sets.
...
\begin{example}[Regular Functions on Distinguished Open Sets]
	\listhack
	\begin{enumerate}[(a)]
		\ii Taking $f=1$ we recover $\OO_V(V) = \CC[V]$ for any affine variety $V$.
		\ii Let $V = \Aff^1$, $U_0 = V \setminus \{0\}$. Then
		\[ \OO_V(U_0) = \left\{ \frac{P(x)}{x^n} \mid P \in \CC[x], \quad n \in \ZZ \right\}. \]
		So more examples are $1/x$ and $(x+1)/x^3$.
	\end{enumerate}
\end{example}
In particular, the theorem implies that for distinguished open sets the ``local representation'' clause
is useless for constructing new functions.
The proof of this theorem requires the Nullstellensatz, so it relies on $\CC$ being algebraically closed.
In fact, a counter-example is easy to find if we replace $\CC$ by $\RR$: consider $\frac{1}{x^2+1}$.

\begin{ques}
	Why doesn't this theorem apply to \Cref{ex:local_rep}?
\end{ques}
...
\chapter{Ringed Spaces and Varieties}
