...
are the \emph{homology groups of the dual cochain complex}.
...
\chapter{Bonus: Cohomology of Manifolds}
In the previous chapter we defined cohomology groups abstractly.
Thus underlies a story that unites algebraic topology and differential geometry.

I haven't talked enough about manifolds to make this precise,
so in this chapter I will mostly be waving my hands.
...
Well, the trick is as follows:
\begin{definition}
	Let $\Omega^k(M)$ denote the real vector space of $k$-forms on $M$.
	For example, $\Omega^0(M)$ is the set of smooth functions $M \to \RR$.
\end{definition}

Now, we can define the exterior derivative $d$ as before, giving a map
\[
	0 \to \Omega^0(M)
	\taking d \Omega^1(M)
	\taking d \Omega^2(M)
	...
	\taking d \dots
\]
and, surprise, this is a chain complex! After all, we know $d^2 = 0$.
So, we can define the homology groups of this chain complex,
call them $\HdR(M)$.

The language that's usually used here: a $k$-form $\alpha$
is called \vocab{closed} if $d\alpha = 0$ (i.e. $\alpha$ is in the kernel),
and \vocab{exact} if $\alpha = d\beta$ for some $\beta$
(i.e.\ $\alpha$ is in the image).
To draw the analogy with the singular case:
\[
...
Thus, 
\begin{moral}
	One can think of elements of cohomology groups
	as differential forms on the space.
\end{moral}
This metaphor can help even for general $H^k(X; G)$.

Why does this happen?
Here is a vague example that gives a sense of how differential
forms can measure the same topological properties (``holes'')
that homology groups were built to detect.
\begin{example}
	[de Rham Cohomology of $S^1$]
	Let $M = S^1$ be a circle (a smooth $1$-manifold).
	One can define the \vocab{angle form} $\alpha$, which measures
	the change in the angle as we move in the circle.

	To be precise, given a point $p \in M$ and a tangent vector $v$ to
	the circle at $M$, $\alpha_p(v)$ gives the singed length of $v$.

	\begin{enumerate}[(a)]
		\ii This form is closed (it is a $1$-form on a one-dimensional space,
		thus $d\alpha = 0$ is clear).

		\ii But it is not exact: there is no function $\theta : M \to \RR$ 
		such that $\alpha = d\theta$.
		The reason is that we can ``integrate'' around the entire circle,
		giving an increment of $2\pi$, and thus we see that it is
		impossible to define $\theta$ globally on $M$.
	\end{enumerate}
	So the failure of $\alpha$ to be exact is signaling
	the fact that $H_1(S^1) \cong \ZZ$.
\end{example}

\section{Poincar\'e Duality}
In this case, the right definition of ``nice'' is 
\emph{compact orientable topological $n$-manifold}.

Since the definitions of cohomology groups I have given are somewhat abstract,
I had better mention Poincar\'e duality, which will give us a sense of
what the cohomology groups $H^k(M, \ZZ)$ look like in the case of a ``nice'' space $M$.
