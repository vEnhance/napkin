...
degree and hairy ball theorem

cellular


\section\problemhead
\begin{sproblem}[Motivating Example for Cohomology]
	Let $X = \CP^2$ and $Y = S^2 \wedge S^4$.
	Prove that $H_n(X) \cong H_n(Y)$ for every $n$.
	(But it turns out that $X$ and $Y$ are not homotopy equivalent.)
\end{sproblem}

\chapter{Singular Cohomology}
Here's the main way to motivate this chapter.
Let $X = \CP^2$ and $Y = S^2 \wedge S^4$;
one can quickly compute that $H_n(X) \cong H_n(Y)$ for every $n$.
